...
#include "../Modules/SafeDashboard.h"
#include <WPILib.h>
#include <iostream>
...
ScriptValidate::ScriptValidate(const char* dashboardInput, const char* dashboardOutput)
...
void ScriptValidate::ValidateCommands() {
	auto commands = SafeDashboard::GetString(_dashboardInput, "S(0)");
	CommandListParser& parser(CommandListParser::GetInstance());
	SmartDashboard::PutString(_dashboardOutput, parser.IsValid(commands) ? "Valid" : "Invalid");
}
